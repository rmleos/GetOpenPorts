function get-OpenPorts {
  param(
      ...
      [Parameter(
        Position=4,
        HelpMessage="Used to export the restuls to CSV file.")]
        [switch]$ExportResults,    
      [Parameter(
        Position=5,
        HelpMessage="Use path with file name to export the restuls to.")]
        [string]$ExportPath,
      [Parameter(
        Position=6,
        HelpMessage="Use switch to show DNS resolution of IP.",
        ValueFromPipelineByPropertyName=$true)]
  ...
  #>
    ...
    $results = $iprange | ForEach-Object -Parallel {
        <#
        ...
        if ($using:TCPPorts) {
            if ($using:ShowOpen)
            {
                $scanresult = port-scan-tcp $_ $using:TCPPorts
            ...
            {
                $result = (port-scan-tcp $_ $using:TCPPorts)
                $result
            }
        }
        ...
        if ($using:UDPPorts) {
            if ($using:ShowOpen)
            {
                $scanresult = port-scan-udp $_ $using:UDPPorts
                foreach ($result in $scanresult)
                {
                    if ($result -match 'Open')
                    {
            ...
            {
                $result += (port-scan-udp $_ $using:UDPPorts)
                $result
            }
        }
     } -ThrottleLimit 300
    ...
    foreach ($client in $results) {
        ...
        $editclient = $client.Split($separator,$option)
        $result = New-Object -TypeName psobject
        $result | Add-Member -MemberType NoteProperty -Name IP -Value $editclient[0]
        $result | Add-Member -MemberType NoteProperty -Name Protocol -Value $editclient[1]
        $result | Add-Member -MemberType NoteProperty -Name Port -Value $editclient[2]
        $result | Add-Member -MemberType NoteProperty -Name Status -Value $editclient[3]
        $editclients += $result
        }
    ...
    if ($DNSresolution)
    {
    foreach ($item in $editclients)
        {
            $dnsresult = New-Object -TypeName psobject
            $dnsresult | Add-Member -MemberType NoteProperty -Name IP -Value $item.IP
            $dnsresult | Add-Member -MemberType NoteProperty -Name Protocol -Value $item.Protocol
            $dnsresult | Add-Member -MemberType NoteProperty -Name Port -Value $item.Port
            $dnsresult | Add-Member -MemberType NoteProperty -Name Status -Value $item.Status
            $dnsresult | Add-Member -MemberType NoteProperty -Name DNS -Value ((Resolve-DnsName $item.IP -DnsOnly -ErrorAction SilentlyContinue).NameHost -join ',')
            $Finalresult += $dnsresult
        }
    }
    else{
        $Finalresult = $editclients
    }
    if ($ExportResults)
     {
       if (!$ExportPath)
       {write-Host -Prompt 'Export Path was not entered.'}
       else {
        $file = $Finalresult
        $file | Export-Csv -Path $ExportPath
      }
    }
